...
import { NEXORA_CHATID, getBaseClasses, getCredentialData, getCredentialParam, parseJsonBody } from '../../../src/utils'
...
class Pinecone_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            ...
            obj.filter.$or = [
                ...(obj.filter.$or || []),
                { [NEXORA_CHATID]: { $eq: options.chatId } },
                { [NEXORA_CHATID]: { $exists: false } }
            ]
        }
