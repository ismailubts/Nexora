...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${NEXORA_CHATID} == "${options.chatId}" OR NOT EXISTS(${NEXORA_CHATID})`
            else milvusFilter = `${NEXORA_CHATID} == "${options.chatId}" OR NOT EXISTS(${NEXORA_CHATID})`
        }
