...
class ExtendedLunaryHandler extends LunaryHandler {
    ...
    constructor({ NEXORAOptions, ...options }: any) {
        super(options)
        this.appDataSource = NEXORAOptions.appDataSource
        this.databaseEntities = NEXORAOptions.databaseEntities
        this.chatId = NEXORAOptions.chatId
        this.apiMessageId = NEXORAOptions.apiMessageId
    }
...
export class AnalyticHandler {
    ...
    async onChainStart(name: string, input: string, parentIds?: ICommonObject) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.handlers, 'arize')) {
            ...
            if (!parentIds || !Object.keys(parentIds).length) {
                rootSpan = tracer ? tracer.startSpan('Nexora') : undefined
                if (rootSpan) {
