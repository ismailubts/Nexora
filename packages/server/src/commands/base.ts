...
export abstract class BaseCommand extends Command {
    static flags = {
        // General Settings
        NEXORA_FILE_SIZE_LIMIT: Flags.string(),
        ...
        DISABLE_NEXORA_TELEMETRY: Flags.string(),
        ...
        NEXORA_SECRETKEY_OVERWRITE: Flags.string(),
        ...
        NEXORA_EE_LICENSE_KEY: Flags.string(),
    ...
    protected onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger.info('Nexora was forced to shut down after 30 secs')
                    await this.failExit()
                }, 30000)

                await this.stopProcess()
            } catch (error) {
                logger.error('There was an error shutting down Nexora...', error)
            }
        }
    }
