import { handleEscapeCharacters, ICommonObject } from 'nexora-components'
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalNEXORAError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
