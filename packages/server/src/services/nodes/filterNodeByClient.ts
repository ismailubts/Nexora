...
// getRunningExpressApp → nexora-components barrel → MCP SDK → pkce-challenge (ESM-only),
// which crashes Jest on Node 18 with "needs ES Modules in the VM API".
// `import type` is erased at compile time, so this file has zero runtime deps on nexora-components.
import type { INode, INodeOptionsValue, INodeParams, ClientType } from 'nexora-components'
