import { webCrawl, xmlScrape, checkDenyList } from 'nexora-components'
...
const getAllLinks = async (requestUrl: string, relativeLinksMethod: string, queryLimit: string): Promise<any> => {
    try {
        ...
        throw new InternalNEXORAError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: fetchLinksService.getAllLinks - ${getErrorMessage(error)}`)
    }
}
