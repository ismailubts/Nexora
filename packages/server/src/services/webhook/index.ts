...
const validateWebhookChatflow = async (
    ...
    try {
        ...
        if (startInputType !== 'webhookTrigger') {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} is not configured as a webhook trigger`)
        }
        ...
        if (enableAuth) {
            ...
            if (!sigValue) {
                throw new InternalNEXORAError(StatusCodes.UNAUTHORIZED, `Missing signature header: ${sigHeader}`)
            }
            ...
            if (!valid) {
                throw new InternalNEXORAError(StatusCodes.UNAUTHORIZED, 'Invalid webhook signature')
            }
        }
        ...
        if (webhookMethod && method?.toUpperCase() !== webhookMethod.toUpperCase()) {
            throw new InternalNEXORAError(StatusCodes.METHOD_NOT_ALLOWED, `Method ${method} not allowed. Expected ${webhookMethod}`)
        }
        ...
        if (enableValidation) {
            ...
            if (missingHeaders.length > 0) {
                throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, `Missing required headers: ${missingHeaders.join(', ')}`)
            }
            ...
            if (typeMismatch.length > 0) {
                throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, `Invalid type for parameter(s): ${typeMismatch.join(', ')}`)
            }
