...
export class WorkspaceUserService {
    ...
    public validateWorkspaceUserStatus(status: string | undefined) {
        if (status && !Object.values(WorkspaceUserStatus).includes(status as WorkspaceUserStatus))
            throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.INVALID_WORKSPACE_USER_SATUS)
    }

    public validateWorkspaceUserLastLogin(lastLogin: string | undefined) {
        if (isInvalidDateTime(lastLogin))
            throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.INVALID_WORKSPACE_USER_LASTLOGIN)
    }
    ...
    public async createWorkspaceUser(data: Partial<WorkspaceUser>, activeOrganizationId: string) {
        ...
        if (workspace.organizationId !== activeOrganizationId)
            throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
        if (workspaceUser) throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, WorkspaceUserErrorMessage.WORKSPACE_USER_ALREADY_EXISTS)
    ...
    public async createWorkspace(data: Partial<Workspace>) {
        ...
        const ownerRole = await this.roleService.readGeneralRoleByName(GeneralRole.OWNER, queryRunner)
        if (!ownerRole) throw new InternalNEXORAError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
        ...
        if (!orgAdmin) throw new InternalNEXORAError(StatusCodes.NOT_FOUND, OrganizationUserErrorMessage.ORGANIZATION_USER_NOT_FOUND)
    ...
    public async updateWorkspaceUser(newWorkspaserUser: Partial<WorkspaceUser>, queryRunner: QueryRunner, activeOrganizationId: string) {
        ...
        if (!workspace || workspace.organizationId !== activeOrganizationId)
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
        if (newWorkspaserUser.roleId && workspaceUser.role) {
            ...
            if (role.organizationId !== workspaceUser.role.organizationId) {
                throw new InternalNEXORAError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
            }
        ...
        if (!updatedBy) throw new InternalNEXORAError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
