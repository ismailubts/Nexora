import { NEXORA_METRIC_COUNTERS, IMetricsProvider } from '../Interface.Metrics'
import express from 'express'
import promClient, { Counter, Histogram, Registry } from 'prom-client'
import { getVersion } from 'nexora-components'
...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'Nexora'
        this.register.setDefaultLabels({
            app: serviceName
        })

        // look at the NEXORA_COUNTER enum in Interface.Metrics.ts and get all values
        ...
        const enumEntries = Object.entries(NEXORA_METRIC_COUNTERS)
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'NEXORA_version_info',
                help: 'Nexora version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
            ...
            this.counters.set('NEXORA_version', versionGaugeCounter)
        } catch (error) {
            // If metric already exists, get it from the registry
            const existingMetric = this.register.getSingleMetric('NEXORA_version')
            if (existingMetric) {
                this.counters.set('NEXORA_version', existingMetric as promClient.Gauge<string>)
            }
        }
    ...
    public incrementCounter(counter: NEXORA_METRIC_COUNTERS, payload: any) {
    ...
    private registerMetrics() {
        if (process.env.METRICS_INCLUDE_NODE_METRICS !== 'false') {
            ...
            promClient.collectDefaultMetrics({
                ...
                prefix: 'NEXORA_' // Add a prefix to avoid conflicts
            })
        }
