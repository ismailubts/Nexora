...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        try {
            ...
            const NEXORAVersion = await getVersion()

            this.resource = new Resource({
                [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'Nexora',
                [ATTR_SERVICE_VERSION]: NEXORAVersion.version // Version as a label
            })
            ...
            const meter = this.meterProvider.getMeter('Nexora-metrics')
            ...
            try {
                ...
                if (!createdMetrics.has('NEXORA_version')) {
                    const versionGuage = meter.createGauge('NEXORA_version', {
                        description: 'Nexora version'
                    })
                    // remove the last dot from the version string, e.g. 2.1.3 -> 2.13 (gauge needs a number - float)
                    const formattedVersion = NEXORAVersion.version.replace(/\.(\d+)$/, '$1')
                    versionGuage.record(parseFloat(formattedVersion))
                    createdMetrics.set('NEXORA_version', true)
                }
            } catch (error) {
