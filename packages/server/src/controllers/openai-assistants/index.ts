...
import { streamStorageFile } from 'nexora-components'
...
const getFileFromAssistant = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!fileStream) throw new InternalNEXORAError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: getFileFromAssistant`)
