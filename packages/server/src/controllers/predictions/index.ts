...
const createPrediction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatflow) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, `Chatflow ${req.params.id} not found`)
        }
        ...
        if (isDomainAllowed) {
            ...
            throw new InternalNEXORAError(StatusCodes.FORBIDDEN, unauthorizedOriginError)
        }
    } catch (error) {
        next(error)
    }
