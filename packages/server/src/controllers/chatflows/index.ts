...
const deleteChatflow = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.id) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, `Error: chatflowsController.deleteChatflow - id not provided!`)
        }
        ...
        if (req.user?.isOrganizationAdmin) {
            ...
            if (userPermittedTypes.length === 0)
                throw new InternalNEXORAError(StatusCodes.FORBIDDEN, `You do not have permission to delete any chatflow types`)
        }
...
const getChatflowById = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.id) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, `Error: chatflowsController.getChatflowById - id not provided!`)
        }
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(
...
const saveChatflow = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.body) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, `Error: chatflowsController.saveChatflow - body not provided!`)
        }
        const orgId = req.user?.activeOrganizationId
        if (!orgId) {
            throw new InternalNEXORAError(
...
const updateChatflow = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (typeof req.params === 'undefined' || !req.params.id) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, `Error: chatflowsController.updateChatflow - id not provided!`)
        }
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(
...
const setWebhookSecret = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.params.id) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, `Error: chatflowsController.setWebhookSecret - id not provided!`)
        }
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.UNAUTHORIZED, `Error: chatflowsController.setWebhookSecret - workspace not found!`)
        }
...
const clearWebhookSecret = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.UNAUTHORIZED, `Error: chatflowsController.clearWebhookSecret - workspace not found!`)
        }
...
const getScheduleStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: chatflowsController.getScheduleStatus - workspace not found!')
        }
...
const getScheduleTriggerLogs = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: chatflowsController.getScheduleTriggerLogs - workspace not found!')
        }
...
const deleteScheduleTriggerLogs = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!Array.isArray(logIds) || logIds.some((x) => typeof x !== 'string')) {
            throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, 'logIds must be a string[]')
        }
...
const toggleScheduleEnabled = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: chatflowsController.toggleScheduleEnabled - workspace not found!')
        }
        const { enabled } = req.body
        if (typeof enabled !== 'boolean') {
            throw new InternalNEXORAError(StatusCodes.BAD_REQUEST, '"enabled" must be a boolean')
        }
