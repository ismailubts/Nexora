...
const getMcpServerConfig = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: mcpServerController.getMcpServerConfig - workspace not found!')
        }
...
const createMcpServerConfig = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: mcpServerController.createMcpServerConfig - workspace not found!')
        }
...
const updateMcpServerConfig = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: mcpServerController.updateMcpServerConfig - workspace not found!')
        }
...
const deleteMcpServerConfig = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: mcpServerController.deleteMcpServerConfig - workspace not found!')
        }
...
const refreshMcpToken = async (req: Request, res: Response, next: NextFunction) => {
    try {
        if (!req.params.id) {
            throw new InternalNEXORAError(StatusCodes.PRECONDITION_FAILED, 'Error: mcpServerController.refreshMcpToken - id not provided!')
        }
        const workspaceId = req.user?.activeWorkspaceId
        if (!workspaceId) {
            throw new InternalNEXORAError(StatusCodes.NOT_FOUND, 'Error: mcpServerController.refreshMcpToken - workspace not found!')
        }
